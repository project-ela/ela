    ...
    pub fn exec(&mut self, opcode: Opcode) {
        match opcode {
            Opcode::AddRm8R8(rm, reg) => {
                self.set_rm(rm, self.cpu.get_register(reg));
            }
            Opcode::AddRm32R32(rm, reg) => {
                let lhs = self.get_rm(rm);
                let rhs = self.cpu.get_register(reg);
            ...
            Opcode::SubR32Rm32(reg, rm) => {
                let lhs = self.cpu.get_register(reg);
                let rhs = self.get_rm(rm);
                let result = (lhs as u64).wrapping_sub(rhs as u64);
                self.update_eflags_sub(lhs, rhs, result);
                self.cpu.set_register(reg, result as u32);
            }
            ...
            Opcode::PushR32(reg) => self.push32(self.cpu.get_register(reg)),
            Opcode::PopR32(reg) => {
            ...
            Opcode::MovRm32R32(rm, reg) => self.set_rm(rm, self.cpu.get_register(reg)),
            Opcode::MovR32Rm32(reg, rm) => self.cpu.set_register(reg, self.get_rm(rm)),
            Opcode::LeaR32M(reg, addr) => {
                self.cpu.set_register(reg, addr as u32);
            }
            Opcode::PopRm32(rm) => {
                let value = self.pop32();
            ...
            Opcode::Nop => {}
            Opcode::MovR32Imm32(reg, value) => self.cpu.set_register(reg, value),
            Opcode::Ret => {
                let value = self.cpu.get_register(Register::EAX);
            ...
            Opcode::IMulRm32(rm) => {
                let lhs = self.cpu.get_register(Register::EAX);
                let rhs = self.get_rm(rm);
                let result = lhs.wrapping_mul(rhs);
                //TODO: update eflags
                self.cpu.set_register(Register::EAX, result);
            }
            Opcode::IDivRm32(rm) => {
                let lhs = self.cpu.get_register(Register::EAX);
                let rhs = self.get_rm(rm);
